...
handle_mime() {
    ...
    audio/*)
        # Path to a temp file to store the previous audio preview PID
        AUDIO_PID_FILE="/tmp/ranger_audio_preview.pid"
        mediainfo "${FILE_PATH}" && exit 5
        exiftool "${FILE_PATH}" && exit 5

        # Kill previous audio preview if running
        ...
        # Start new preview and store its PID
        (mpv --no-video --quiet "$FILE_PATH" &>/dev/null & echo $! > "$AUDIO_PID_FILE") &
        mediainfo "${FILE_PATH}" && exit 5
        exiftool "${FILE_PATH}" && exit 5
        sleep 1
        exit 1;;
